	...
	for (var i = 0; i < currentData.length; i++) { 
		...
		if (lat){
			...
			var marker = L.marker([lat,lon], {icon: icon}).bindPopup(
				currentData[i].address + "<br>" + "<b>Primary Violation: </b>" + currentData[i].primary_reported_violation + "<br>" + "<b>Property Type: </b>" + currentData[i].property_type + "<br>" + "<b>Status: </b>" + currentData[i].status + "</br></br><img src='https://maps.googleapis.com/maps/api/streetview?size=200x200&location=" + lat + "," + lon + "'/>").addTo(markers);
		}
	}
	markers.addTo(map);
}
	...
	for (var i = 0; i < currentData.length; i++) { 
		...
		if (lat){
			...
			var marker = L.marker([lat,lon], {icon: icon, maxWidth: 500}).bindPopup(
				currentData[i].address + "<br>" + "<b>Primary Violation: </b>" + currentData[i].primary_reported_violation + "<br>" + "<b>Property Type: </b>" + currentData[i].property_type + "<br>" + "<b>Status: </b>" + currentData[i].status + "</br></br><img src='https://maps.googleapis.com/maps/api/streetview?size=200x200&location=" + lat + "," + lon + "'/>"
				).addTo(markers);
		}
	}
	markers.addTo(map);
...
$( ".property_type" ).children().click(function() {
	...
	filterList["property_type"] = filterValue;
	superFilter();
});
...
function identifyCVs (){
	...
	for (var i = 0; i < data2.length; i++){
		var folderRsn = data2[i].folder_id;
