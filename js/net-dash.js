...
//new for v1.2.2//
//actually link to bootstrap.js
//collapse dash filter buttons into dropdown
//add (one) inspector name (in progress)
//info bubble tooltips 
//brower check and alert if explorer

//new for v1.1
// 1.	Charts – replaced ‘Deficiencies’ chart with a chart of how many violation cases have been created
// 2.	Data table – sort columns in descending or ascending order by clicking on column header
// 3.	Data table – search on any attribute (e.g., type in a street name, a case status, or a violation type)
// 4.	Data table – active cases highlighted in red, closed in green, etc.
//5. add info bubbles...in progress

//^^^^^that's is for version1.0^^^^^
//
//date updated only reflects the CC
//move preparestimestack chaart data porttion to the group data objects function
//major cleanup > move time mappings to datapicker
//duration variable a constant var
//check your status groupdata-ing....it's hardcoded and probably vulnerable to showing bad data
//make browser alert a div
...
var scaleDict = {"chart_1": "","chart_2": "","chart_4": ""};
...
function groupDataObjects(){  // see http://learnjsdata.com/group_data.html
	...
	var primaryViolationStatusCountProperty= d3.nest()
		.key(function(d) { return d.case_type; }) //property is highest level obj
	...
	deficiencyStatusCount = d3.nest()
		.key(function(d) { return d.status; }) //status is highest level obj
		.key(function(d) { return d.categorydesc; }) //within status is each violation type
		.rollup(function(v) { return v.length; }) //rolling up the number of records for each violation type/status
...
//And this is categorydesc types by property type
	deficiencyStatusCountProperty = d3.nest()
		.key(function(d) { return d.case_type; }) //status is highest level obj
		.key(function(d) { return d.status; }) //status is highest level obj
		.key(function(d) { return d.categorydesc; }) //within status is each violation type
		.rollup(function(v) { return v.length; }) //rolling up the number of records for each violation type/status
		.map(data2);//note that .map gives you key:value compared to .entries which gives you key: [the key], value: [the value]
	...
	for (var i = 0; i < statusList.length; i++){
		...
		dataPicker.CCs["statuses"].push({"status": statusList[i],"all":statusCountCC[statusList[i]]})
	...
	var statusCountType = d3.nest().key(function(d) { return d.case_type; }) .rollup(function(v) { return v.length; })	.map(data1);
	for (var i = 0; i < typeList.length; i++){
	...
	for (var i = 0; i < typeList.length;i++) {
		...
		if (typeList[i]=="Multifamily") { counter = 1;} else { counter = 0};
		dataPicker.CCs.typeCounts[i]["Multifamily"] = counter; //to handle button clicks
		if (typeList[i]=="Commercial") { counter = 1;} else { counter = 0};
		dataPicker.CCs.typeCounts[i]["Commercial"] = counter; 
		if (typeList[i]=="Neighborhood") { counter = 1;} else { counter = 0};
...
function makeObjectsEqual(dataObject, keyListReference){ //make all nested objects within an object the same length (or D3.layout.stack will fail)
	...
	if (typeof keyListReference === 'undefined') { keyListReference = 0; } //default keyListReference
...
function formatForD3Stack(dataset) {
	...
	for (var i = 0; i < statusList.length; i++){ //for every status type (here, hardcoded as active or closed to avoid sorting issues)  < still true? 7/20/2015
		...
		for (var q = 0; q< localKeyList.length; q++){ //for every key in each object key 
			...
			temp.push({"x":name, "y":count, "status": status});
		}
	...
	var xAxis = d3.svg.axis()
		...
		.tickFormat(function(d) { 
			if (divId == "chart_4") {  //alternate labels for time chart
				...
				} else { alternateLabel = true; return ""}
			} else {
				var xLabel = dataset[0][d].x;
				return xLabel.trunc(10, false);
			}
...
function updateBarChart(divId, dataset, duration, yScale) {
	if (typeof duration === 'undefined') { duration =750; } //default duration can be overridden
...
function makePieChart(dataset, divId){
	...
	pie = d3.layout.pie()
		.sort(null)
		.value(function(d) { return d.fake; });
	...
	path1 = svg.datum(dataset).selectAll("path")
		...
		.append("path").attr("class", function(d) {return (d.data.status);})
		.attr("d", arc)
...
function updatePieChart(dataType, dataset, duration) {
	if (typeof duration === 'undefined') { duration =500; } //default duration can be overridden
	var value = dataType;
	...
	arcData = [];
	d3.selectAll("#pie").selectAll("path").each(function(d) { arcData.push(d)}) //get new path data
	//move text
...
function prepareTimeStackChart(dataset){
	...
	masterDict = {"all":{}, "Neighborhood":{},"Multifamily":{},"Commercial":{}} //expand to include mf, comm, nb...
	...
	for (var i = 0; i < dataset.length; i++){
		...
		} else { lastUpdated = openedDay; } //if case has never been updated, last update is opened date
